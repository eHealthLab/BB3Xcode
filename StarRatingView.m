...
- (id)initWithFrame:(CGRect)frame andRating:(int)rating withLabel:(BOOL)label animated:(BOOL)animated
{
    ...
    if (self) {
        ...
        if (label) {
            ...
            //NSLog(@"inside star, starting with: %@", appDelegate.messagesRatingLibrary[appDelegate.currentMessageIndex]);
            
            self.label.text = appDelegate.messagesRatingLibrary[appDelegate.currentMessageIndex];
            
            //self.label.text = [NSString stringWithFormat:@"%d",rating/20];
...
-(void)increaseRating
{
    ...
    if (_rating<_maxrating) {
        ...
        if (self.label) {
            ...
            //self.label.text = [NSString stringWithFormat:@"%d",self.rating/20];
            appDelegate.messagesRatingLibrary[appDelegate.currentMessageIndex] = self.label.text;
            //NSLog(@"inside star, changed to: %@", appDelegate.messagesRatingLibrary[appDelegate.currentMessageIndex]);
        }
    }else{
        [timer invalidate];
    }
