...
-(void)viewDidLoad
{
    ...
    int shortByPoints = 150 - appDelegate.totalPoints;
    NSString *string = [NSString stringWithFormat:@"%d", shortByPoints];
    NSString *message = [[@"You are " stringByAppendingString:string] stringByAppendingString:@" points short to earn your first badge. Login more minutes."];
    
    if (appDelegate.totalPoints < 150) {
        UIAlertView *messageAlert = [[UIAlertView alloc]
                                     initWithTitle:@"Note!" message:message delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [messageAlert show];
    }
    
    else if (appDelegate.totalPoints >= 150)
    {
